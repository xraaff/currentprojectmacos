...
async def extract_messages(chat_id_or_link, limit=100):
    ...
    async for message in client.iter_messages(entity, limit=limit):
        ...
        messages.append({
            ...
            "media_path": media_path  # Сохраняем путь к медиафайлу
        })
        print(f"Собрано сообщение ID: {message.id}")

    # Сохраняем сообщения в JSON-файл
    with open(f"{entity.title}_messages.json", "w", encoding="utf-8") as file:
        json.dump(messages, file, ensure_ascii=False, indent=4)
    print(f"Сохранено {len(messages)} сообщений в файл: {entity.title}_messages.json")
...
with client:
    ...
    client.loop.run_until_complete(extract_messages(chat_id_or_link, limit))https://t.me/romangorbachevhttps://t.me/romangorbachev